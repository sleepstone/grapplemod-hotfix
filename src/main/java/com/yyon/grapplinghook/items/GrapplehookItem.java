...
public class GrapplehookItem extends Item implements KeypressItem {
    ...
    public GrapplehookItem() {
        super(new Properties().stacksTo(1).durability(GrappleConfig.getConf().grapplinghook.other.default_durability));
    }
    ...
    public void onCustomKeyDown(ItemStack stack, Player player, Keys key, boolean ismainhand) {
        if (player.level().isClientSide) {
            ...
            if (key == Keys.THROWBOTH || (!custom.doublehook && (key == Keys.THROWLEFT || key == Keys.THROWRIGHT))) {
