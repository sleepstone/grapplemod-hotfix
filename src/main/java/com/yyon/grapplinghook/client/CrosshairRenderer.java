...
public class CrosshairRenderer {
    ...
    public void onRenderGameOverlayPost(RenderGuiOverlayEvent.Post event) {
        ...
        if (gamesettings.renderDebug && !gamesettings.hideGui && !this.mc.player.isReducedDebugInfo() && !gamesettings.reducedDebugInfo().get())
            return;
...
//	            RenderSystem.enableTexture();
    ...
    public void drawTexturedModalRect(int x, int y, int textureX, int textureY, int width, int height) {
        ...
        bufferbuilder.vertex(x, y + height, this.zLevel).uv(((float) (textureX) * f), ((float) (textureY + height) * f1)).endVertex();
        bufferbuilder.vertex(x + width, y + height, this.zLevel).uv(((float) (textureX + width) * f), ((float) (textureY + height) * f1)).endVertex();
        bufferbuilder.vertex(x + width, y, this.zLevel).uv(((float) (textureX + width) * f), ((float) (textureY) * f1)).endVertex();
        bufferbuilder.vertex(x, y, this.zLevel).uv(((float) (textureX) * f), ((float) (textureY) * f1)).endVertex();
        tessellator.end();
    }
...
//        GL11.glLineWidth(4.0F);
        bufferbuilder.begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION_COLOR);
        bufferbuilder.vertex(x, y + height, this.zLevel).color(g, g, g, a).endVertex();
        bufferbuilder.vertex(x + width, y + height, this.zLevel).color(g, g, g, a).endVertex();
        bufferbuilder.vertex(x + width, y, this.zLevel).color(g, g, g, a).endVertex();
        bufferbuilder.vertex(x, y, this.zLevel).color(g, g, g, a).endVertex();
//        bufferbuilder.end();
